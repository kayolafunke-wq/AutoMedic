  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
          <h1 className="font-display text-2xl text-[#1A1A2E]">Inventory Tracking</h1>
      ...
      <div className="flex gap-1 bg-gray-100 p-1 rounded-xl mb-5 w-fit">
        {[['ledger','📋 Movement Ledger'],['summary','📊 Product Summary']].map(([key, label]) => (
      ...
      {/* ── ADJUST MODAL ───────────────────────────────────────────────── */}
      {adjustModal && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setAdjustModal(null)}>
          <div className="bg-white rounded-2xl p-7 max-w-sm w-full shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">{adjustModal.name}</p>
              </div>
              <button onClick={() => setAdjustModal(null)} className="w-8 h-8 bg-gray-100 rounded-full flex items-center justify-center hover:bg-gray-200">
                <X size={14} />
              </button>
            </div>
            ...
            <div className="space-y-4">
              ...
              <button onClick={adjust} disabled={adjSaving}
                className="w-full py-3.5 bg-[#B8860B] text-white font-bold rounded-xl hover:bg-[#8B6508] transition-all disabled:opacity-60 text-sm flex items-center justify-center gap-2">
                {adjSaving
