...
export default function ServicesManagement() {
  ...
  const uploadImageBase64 = async (serviceId, base64Data) => {
    setUploading(true)
    try {
      const res = await api.post(`/services/${serviceId}/image`, { image_data: base64Data })
  ...
  const removeImage = async (serviceId) => {
    try {
      ...
      setServices(prev => prev.map(s => s.id === serviceId ? { ...s, image_url: null } : s))
      setImagePreview(null)
      setImageFile(null)
    ...
    try {
      ...
      if (modal === 'add') {
        ...
        if (imageFile && imagePreview) {
          const newUrl = await uploadImageBase64(service.id, imagePreview)
          if (newUrl) service = { ...service, image_url: newUrl }
        }
        
        setServices(prev => [service, ...prev])
        showToast(`"${form.name}" added successfully`)
      } else {
        // Save service details first
        const patchRes = await api.patch(`/services/${modal.id}`, payload)
        service = patchRes.data.data
