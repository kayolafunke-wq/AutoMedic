...
router.delete('/:id/permanent', authenticate, authorize('admin'), async (req, res) => {
  try {
    ...
    const userRow = await db.query('SELECT id, name, role FROM users WHERE id = ?', [id])
    ...
    const [appts, vehicles, invoices, notifications] = await Promise.all([
      db.query('SELECT COUNT(*) as cnt FROM appointments WHERE customer_id = ?', [id]),
      db.query('SELECT COUNT(*) as cnt FROM vehicles WHERE customer_id = ?', [id]),
      db.query('SELECT COUNT(*) as cnt FROM invoices WHERE customer_id = ?', [id]),
      db.query('SELECT COUNT(*) as cnt FROM notifications WHERE user_id = ?', [id]),
    ])
    ...
    await db.query('DELETE FROM invoices WHERE customer_id = ?', [id])

    // Job cards assigned to this technician — nullify, don't delete (preserve history)
    await db.query("UPDATE job_cards SET technician_id = NULL WHERE technician_id = ?", [id])

    // Appointments & their related records
    const userAppts = await db.query('SELECT id FROM appointments WHERE customer_id = ?', [id])
    for (const a of userAppts.rows) {
      // repair_updates → job_cards → inspections → inspection_photos
      const jcRows = await db.query('SELECT id FROM job_cards WHERE appointment_id = ?', [a.id])
      for (const jc of jcRows.rows) {
        await db.query('DELETE FROM repair_updates WHERE job_card_id = ?', [jc.id])
      }
      await db.query('DELETE FROM job_cards WHERE appointment_id = ?', [a.id])

      const inspRows = await db.query('SELECT id FROM inspections WHERE appointment_id = ?', [a.id])
      for (const insp of inspRows.rows) {
        await db.query('DELETE FROM inspection_photos WHERE inspection_id = ?', [insp.id])
      }
      await db.query('DELETE FROM inspections WHERE appointment_id = ?', [a.id])
    }
    await db.query('DELETE FROM appointments WHERE customer_id = ?', [id])

    // Vehicles
    await db.query('DELETE FROM vehicles WHERE customer_id = ?', [id])
